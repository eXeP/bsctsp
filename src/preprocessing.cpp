...
one_tree prim_onetree_edges_ij(std::vector<std::vector<float>>& p, std::vector<float>& pi, int is, int js) {
    int n = p[0].size();
...
std::vector<std::vector<float>> calculate_exact_alpha(std::vector<std::vector<float>>& coords, std::vector<float>& pi) {
    auto T = prim_onetree_edges(coords, pi);
    int n = coords[0].size();
    std::vector<std::vector<float>> alpha(n, std::vector<float>(n, 0));
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            auto T_PLUS_IJ = prim_onetree_edges_ij(coords, pi, i, j);
            alpha[i][j] = T_PLUS_IJ.length - T.length;
        }
    }
    return alpha;
}
